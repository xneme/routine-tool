...
must_haves:
  truths:
    ...
    - "Overdue tasks can be quickly rescheduled via Tomorrow or +1 week buttons"
...
<task type="auto">
  ...
  <action>
    ...
    **TaskListViewModel.kt:**
    ...
    - `rescheduleTask(taskId: String, newDate: LocalDate)` — update the nearest deadline (soft or hard, whichever was overdue) to the new date
  ...
  <action>
    **TaskListScreen.kt:**
    ...
    fun TaskListScreen(
        ...
        onEditTask: (String) -> Unit = {},  // Edit callback, wired in Plan 03
    ...
    **Expanded state** (shown when isExpanded = true):
    ...
    - If isLongOverdue: gentle text "This has been waiting a while" with quick-reschedule buttons: "Tomorrow", "+1 week" as compact TextButtons, plus "Remove deadline" — neutral tone, no judgment
    - If isOverdue but not long: "X days ago" badge plus quick-reschedule buttons: "Tomorrow", "+1 week" as compact TextButtons
    - Quick-reschedule buttons call `onReschedule(taskId, newDate)` to update the nearest deadline to the chosen date
    - "Remove deadline" action (calls onDismissOverdue) removes both deadlines entirely

    - **Edit button:** TextButton labeled "Edit" at the bottom of expanded content — calls `onEditTask()` which navigates to the edit screen (wired in Plan 03)
